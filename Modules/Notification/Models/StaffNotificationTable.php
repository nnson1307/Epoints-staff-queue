...
class StaffNotificationTable extends Model
{
    ...
    protected $fillable = [
        ...
        'notification_message', 'notification_type', 'is_read', 'staff_id','is_new'
    ];
    ...
    public function addNotify($idUser, $title, $message, $detailId = null, $avatar = null)
    {
        return self::create([
            'staff_id' => $idUser,
            'staff_notification_detail_id' => $detailId,
    ...
    /**
     * Đếm số lượng notification mới
     *
     * @param $idUser
     * @return mixed
     */
    public function countIsNewNotify($idUser)
    {
        return $this->where('staff_id', $idUser)
            ->where('is_new', 0)
            ->count();
