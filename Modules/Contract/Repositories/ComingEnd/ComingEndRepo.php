...
class ComingEndRepo implements ComingEndRepoInterface
{
    /**
    ...
    public function jobSaveLogComingEnd()
    {
        try {
            ...
            $getComingEnd = $mContractRemind->getRemindComingEnd();

             if (count($getComingEnd) > 0) {
                foreach ($getComingEnd as $v) {
                    //Xử lý nhắc nhở sắp đến hạn HĐ
                    $this->handleComingEnd($v);
                }
             }
        } catch (\Exception $e) {
    ...
    /**
     * Xử lý dữ liệu sắp hết hạn HĐ
     *
     * @param $input
     * @throws \Psr\Container\ContainerExceptionInterface
     * @throws \Psr\Container\NotFoundExceptionInterface
     */
    public function handleComingEnd($input)
    {
        ...
        if (count($getContract) > 0) {
            ...
            foreach ($getContract as $v) {
                ...
                if ($v['expired_date'] != null && $v['is_renew'] == 1) {
