...
class WarrantyExpiredRepo implements WarrantyExpiredRepoInterface
{
    /**
     * Job nhắc nhhở HĐ đến hạn bảo hành
     *
     * @return mixed|string
    ...
    /**
     * XỬ lý dữ liệu nhắc nhở hết hạn bảo hành
     *
     * @param $input
     * @throws \Psr\Container\ContainerExceptionInterface
     * @throws \Psr\Container\NotFoundExceptionInterface
     */
    public function handleWarrantyExpired($input)
    {
        ...
        if (count($getContract) > 0) {
            ...
            foreach ($getContract as $v) {
                ...
                if ($v['warranty_end_date'] != null) {
                    $dateCompare = Carbon::parse($v['warranty_end_date'])->format('Y-m-d');
                }
